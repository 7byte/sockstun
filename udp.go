...
func udpLocal(laddr, server, target string, shadow func(net.PacketConn) net.PacketConn) {
	...
	if err != nil {
		logger.Warning("UDP server address error: %v", err)
		return
	}
	...
	if tgt == nil {
		...
		logger.Warning("UDP target address error: %v", err)
		return
	}

	c, err := net.ListenPacket("udp", laddr)
	if err != nil {
		logger.Warning("UDP local listen error: %v", err)
		return
	}
	defer c.Close()
	...
	logger.Info("UDP tunnel %s <-> %s <-> %s", laddr, server, target)
	for {
		n, raddr, err := c.ReadFrom(buf[len(tgt):])
		if err != nil {
			logger.Warning("UDP local read error: %v", err)
			continue
		}
		...
		if err != nil {
			logger.Warning("UDP local write error: %v", err)
			continue
		}
	}
}
...
func udpSocksLocal(laddr, server string, shadow func(net.PacketConn) net.PacketConn) {
	...
	for {
		...
		if pc == nil {
			...
			logger.Info("UDP socks tunnel %s <-> %s <-> %s", laddr, server, socks.Addr(buf[3:]))
			pc = shadow(pc)
...
func udpRemote(host string, port int, shadow func(net.PacketConn) net.PacketConn) {
	...
	logger.Info("listening UDP on %s", addr)
	for {
		n, raddr, err := c.ReadFrom(buf)
		if err != nil {
